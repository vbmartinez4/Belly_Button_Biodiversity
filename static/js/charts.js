...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultSampleArray = sampleSamples.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    var results = resultSampleArray[0];
    ...
    var barData = {
      ...
      text: labels,
      orientation: "h",
      marker: {color:"rgba(245,48,39,0.6)"}
    };

    //var trace = samples.json

    // 9. Create the layout for the bar chart. 
    var barLayout = {
     title: "Top 10 Bacteria Cultures Found",
     paper_bgcolor: "rgba(210, 210, 210,0.6)",
     plot_bgcolor: "rgba(210, 210, 210,0.6)"   
    };
...
// Create a Gauge Chart
    ...
    var washFrequency = parseInt(metaResults.wfreq);
